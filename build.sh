...
then
 ...
 This is a GUI application that can generate Kenwood DAP databases on a selected FAT32 formatted USB drive. The database is used by Kenwood car audio systems to allow searching by album, title, genre and artist. It also allows creation of playlists."
    ...
    --deb-changelog "$thisScriptDir/packaging/deb/unstable/debian/changelog" -d "python3-pyqt5 >= 5.5~" -d "python3-venv >= 3.5~" -d "python3-wheel >= 0.29~" --deb-recommends "qttranslations5-l10n >= 5.5~" \
    --after-install "$thisScriptDir/packaging/deb/unstable/debian/postinst.ex" --before-remove "$thisScriptDir/packaging/deb/unstable/debian/prerm.ex" .
